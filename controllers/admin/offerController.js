...
const offer = async (req, res) => {
  try { 
    ...
    const offerData = await offerDB.find({block:false}).limit(limit)  //limit
    .skip(startIndex)  
    .exec();//fetching data form DB which are only blocked === false
    res.render("admin/offer", { productData, categoryData, offerData , page , maxPage,});
  } catch (error) {
    console.error(error);
...
const offerEditPost = async(req,res)=>{
  const offerId = req.params.offID
  console.log(offerId);
  const { couponName, couponValue, category, product, validFrom, validTo } =  req.body;
