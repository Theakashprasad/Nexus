...
const coupon = async (req, res) => {
  try {
    ...
    const coupondata = await couponDB
      .find({ validTo: { $gte: curentDate } })
      .limit(limit) //limit
      .skip(startIndex)
...
const editCouponPost = async (req, res) => {
  try {
    ...
    const {
      ...
      validTo,
      discountType,
    } = req.body; //extracting data form the form data
    // console.log(couponName,couponValue,couponMinimumPurchase,validFrom,validTo,discountType,"updated coupon")
    const isCouponAvailable = await couponDB.findOne({
      couponName: couponName,
    }); //find the coupon exiting or not
    if (!isCouponAvailable) {
      const isCouponUpdated = await couponDB.findOneAndUpdate(
        //updateing the coupon , 'edting'
        { _id: couponId },
        {
          $set: {
            couponName,
      ...
      req.session.errmsg = "alredy exist"; //eroor
      res.redirect("back");
    }
  } catch (error) {
    console.error("Error creating user:", error);
