...
const addProduct = async (req, res) => {
  try {
    if (!req.session.invalid) {
      ...
      let message = "";
      res.render("admin/addProduct", { displayCategory, message }); //rendering the data
    } else {
  ...
  try {
    ...
    const user = await productDB.findByIdAndUpdate(
      ...
      userId,
      {
        ...
        size: [
          quantity1 == "" ? data.size[0] : quantity1,
          quantity2 == "" ? data.size[1] : quantity2,
          quantity3 == "" ? data.size[2] : quantity3,
          quantity4 == "" ? data.size[3] : quantity4,
        ],
